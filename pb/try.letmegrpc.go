...
func NewHandler(grpcAddr string, stringer func(req, resp interface{}) ([]byte, error), opts ...google_golang_org_grpc.DialOption) (net_http.Handler, error) {
	...
	TryServiceClient := NewTryServiceClient(conn)
	TryServiceServer := NewHTMLTryServiceServer(TryServiceClient, stringer)
	mux.HandleFunc("/TryService/Notify", TryServiceServer.Notify)
	mux.HandleFunc("/TryService/LastNotify", TryServiceServer.LastNotify)
	return mux, nil
}

type htmlTryService struct {
	client   TryServiceClient
	stringer func(req, resp interface{}) ([]byte, error)
}

func NewHTMLTryServiceServer(client TryServiceClient, stringer func(req, resp interface{}) ([]byte, error)) *htmlTryService {
	return &htmlTryService{client, stringer}
}

var FormTryService_Notify string = `<div class="container"><div class="jumbotron">
	<h3>TryService: Notify</h3>
...
var nodeFactory = {"TryRequest_RootKeyword": buildTryRequest_RootKeyword(emptyIfNull(null)),}
	function buildTryRequest_RootKeyword(json) {
if (json == undefined) {
		return "";
	}
	
var s = '<div class="node" type="TryRequest_RootKeyword" fieldname="RootKeyword" repeated="false">';
s += '<div class="field form-group"><label class="col-sm-2 control-label">id: </label><div class="col-sm-10"><input class="form-control" name="id" type="text" '+setStrValue("", json["id"])+'/></div></div>';
	...
	var root = $(nodeFactory["TryRequest_RootKeyword"]);
	...
	$("#form > .children").html(buildTryRequest_RootKeyword(json));
	activateLinks(root);
	$("a[id=submit]").click(function(ev) {
		ev.preventDefault();
		c = getChildren($("#form"));
		j = JSON.stringify(c["RootKeyword"]);
		window.location.assign("./Notify?json="+j);
	});
}
...
func (this *htmlTryService) Notify(w net_http.ResponseWriter, req *net_http.Request) {
	w.Write([]byte(Header(`TryService`, `Notify`)))
	...
	w.Write([]byte(FormTryService_Notify))
	if someValue {
		down, err := this.client.Notify(golang_org_x_net_context.Background(), msg)
		if err != nil {
			if err != io.EOF {
				w.Write([]byte("<div class=\"alert alert-danger\" role=\"alert\">" + err.Error() + "</div>"))
				return
...
var FormTryService_LastNotify string = `<div class="container"><div class="jumbotron">
	<h3>TryService: LastNotify</h3>
...
s += '<div class="field form-group"><label class="col-sm-2 control-label">id: </label><div class="col-sm-10"><input class="form-control" name="id" type="text" '+setStrValue("", json["id"])+'/></div></div>';
	...
	$("a[id=submit]").click(function(ev) {
		...
		c = getChildren($("#form"));
		j = JSON.stringify(c["RootKeyword"]);
		window.location.assign("./LastNotify?json="+j);
	});
}
...
func (this *htmlTryService) LastNotify(w net_http.ResponseWriter, req *net_http.Request) {
	w.Write([]byte(Header(`TryService`, `LastNotify`)))
	jsonString := req.FormValue("json")
	someValue := false
	msg := &TryRequest{}
	if len(jsonString) > 0 {
	...
	w.Write([]byte(FormTryService_LastNotify))
	if someValue {
		reply, err := this.client.LastNotify(golang_org_x_net_context.Background(), msg)
		if err != nil {
			if err != io.EOF {
				w.Write([]byte("<div class=\"alert alert-danger\" role=\"alert\">" + err.Error() + "</div>"))
				return
